...
export default function InspectionWorkspace() {
  ...
  // edited later, but only by whoever originally issued it or an
  // administrator — everyone else with certificates.edit falls back to
  // the same view-only preview as someone with no edit permission at
  // all. The backend enforces this independently (see certificates.py's
  // save_certificate) and is the real authority; this is what keeps the
  // UI from showing an editable form for a save that would just get
  // rejected as 403.
  const isLockedToCreator = current.status === "final" && !!current.issuedById && user?.role !== "admin" && current.issuedById !== user?.id;
      ...
      <div className="inspections-page" data-type={type}>
        ...
        <div style={{ padding: "10px 20px 0" }}>
          <div style={{ background: "#E7ECF1", border: "1px solid #455A73", borderRadius: 6, padding: "8px 12px", fontSize: 12, color: "#243040" }}>
            {isLockedToCreator
              ? `View-only — this certificate was finalized by ${current.issuedBy || "another user"}. Only they or an administrator can edit it further; you can still view and print it here.`
