...
def save_certificate(
    ...
    if existing:
        ...
        # the person who originally created it or an administrator —
        # anyone else with certificates.edit gets 403 here, and the
        # frontend keeps them on the same view-only preview it already
        # shows for someone with no edit permission at all (see
        # InspectionWorkspace.tsx's isLockedToCreator). Matches Finance's
        # own _can_edit rule (api/routes/finance.py) for the same reason:
        # an admin needs to be able to fix any record, not just their own.
        # Checked against the record's CURRENT status (before this save
        # applies), not the incoming one, since a currently-final
        # certificate is what needs protecting — an edit that also
        # happens to change status has already gotten this far
        ...
        # unchanged number).
        if existing.status == "final" and current_user.role != "admin" and existing.issued_by_id != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="This certificate has already been finalized. Only the person who originally issued it, or an administrator, can edit it further.",
            )
